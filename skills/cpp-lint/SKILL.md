...
**排除目录**：项目根目录存在`.gitignore`则跳过对应的目录, 并且自动跳过 `build/`、`cmake-build-*/`、`third_party/`、`vendor/`、`node_modules/`、`.git/`等非项目源码目录。
...
**Windows (PowerShell)**：
```powershell
$files = Get-ChildItem -Recurse -Include *.cpp,*.h,*.hpp,*.cc,*.cxx,*.hh -Exclude build,cmake-build-*,third_party,vendor,node_modules,.git
$errCount = 0
foreach ($f in $files) {
...
2. 定位或确认 `.clang-format` 配置
3. 确定扫描范围(CMakeLists 项目从根目录递归扫描,或按用户指定)
...
- 配置文件优先级：`--style` 参数 > `.clang-format`(当前目录)> `.clang-format`(上级目录逐级向上)> 默认 LLVM 风格
